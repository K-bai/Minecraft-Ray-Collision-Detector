...
execute unless entity @e[tag=wrnmd_target_entity_temp] at @s positioned ^ ^ ^1.7 positioned ~-.05 ~-.05 ~-.05 if entity @e[type=#wrnmd:target,dx=0,dy=0,dz=0] positioned ~-.9 ~-.9 ~-.9 run tag @e[type=#wrnmd:target,dx=0,dy=0,dz=0] add wrnmd_target_entity_temp
# 加tag
...
execute at @e[tag=wrnmd_target_entity_temp] run tp @s ~ ~ ~
