...
execute if entity @s[tag=mrcd_touch_edge_complex] run function mrcd:generic/complex_cube/convert
...
execute if score #if_reach_target mrcd_system matches 1 run function mrcd:generic/reach_target
### 如果总距离大于目标距离，且碰撞方块，表示到达终点
### #total_x,y,z >= #target_c_x,y,z and collide with a block ----> Collide with a block
execute if score #if_reach_target mrcd_system matches 0 if entity @s[tag=mrcd_touch_edge] run function mrcd:generic/hit_block
### 如果总距离大于目标距离，且未碰撞方块，表示还未到达终点
### #total_x,y,z >= #target_c_x,y,z and not collide with a block ----> Next cycle
execute if score #if_reach_target mrcd_system matches 0 if entity @s[tag=!mrcd_touch_edge] run function mrcd:generic/next_cycle
