# 修改#total
scoreboard players operation #total_x mrcd_system -= #target_c_x mrcd_system
scoreboard players operation #total_y mrcd_system -= #target_c_y mrcd_system
scoreboard players operation #total_z mrcd_system -= #target_c_z mrcd_system
# 修改#block
...
# 改变执行坐标继续递归 Change execute position and continue
execute if score #target_x mrcd_system matches 1000 positioned ~1 ~ ~ run function mrcd:generic/recurse
execute if score #target_x mrcd_system matches -1 positioned ~-1 ~ ~ run function mrcd:generic/recurse
execute if score #target_y mrcd_system matches 1000 positioned ~ ~1 ~ run function mrcd:generic/recurse
execute if score #target_y mrcd_system matches -1 positioned ~ ~-1 ~ run function mrcd:generic/recurse
execute if score #target_z mrcd_system matches 1000 positioned ~ ~ ~1 run function mrcd:generic/recurse
execute if score #target_z mrcd_system matches -1 positioned ~ ~ ~-1 run function mrcd:generic/recurse