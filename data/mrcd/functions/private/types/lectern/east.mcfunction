# top block
scoreboard players set #box_x0 mrcd_system 125
scoreboard players set #box_x1 mrcd_system 391
scoreboard players set #box_y0 mrcd_system 875
scoreboard players set #box_y1 mrcd_system 1000
scoreboard players set #box_z0 mrcd_system 0
scoreboard players set #box_z1 mrcd_system 1000
function mrcd:private/complex_cube/main

# mid block
scoreboard players set #box_x0 mrcd_system 391
scoreboard players set #box_x1 mrcd_system 657
scoreboard players set #box_y0 mrcd_system 750
scoreboard players set #box_y1 mrcd_system 1000
scoreboard players set #box_z0 mrcd_system 0
scoreboard players set #box_z1 mrcd_system 1000
function mrcd:private/complex_cube/main

# bottom block
scoreboard players set #box_x0 mrcd_system 657
scoreboard players set #box_x1 mrcd_system 938
scoreboard players set #box_y0 mrcd_system 625
scoreboard players set #box_y1 mrcd_system 875
scoreboard players set #box_z0 mrcd_system 0
scoreboard players set #box_z1 mrcd_system 1000
function mrcd:private/complex_cube/main